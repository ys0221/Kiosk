...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        int num_beer = 2;
        int num_customer = 1;
        double[] burger_price = new double[5];
        double total_burger_price = 0.0;
        double[] frozen_custard_price = new double[6];
        double total_frozen_custard_price = 0.0;
        double[] drink_price = new double[6];
        double total_drink_price = 0.0;
        double[] beer_price = new double[2];
        ...
        while (true) {
            ...
            while (select_menu > 6 || select_menu < 1) {
            ...
            if (select_menu == 1) {
                ...
                while (select_burger > 5 || select_burger < 1) {
                    System.out.println("잘못 선택하셨습니다. 다시 선택해주세요");
            ...
            if (select_menu == 2) {
                ...
                while (select_frozen_custard > 6 || select_frozen_custard < 1) {
                    System.out.println("잘못 선택하셨습니다. 다시 선택해주세요");
            ...
            if (select_menu == 3) {
                ...
                while (select_drink > 6 || select_drink < 1) {
                    System.out.println("잘못 선택하셨습니다. 다시 선택해주세요");
            ...
            if (select_menu == 4) {
                ...
                while (select_beer > 2 || select_beer < 1) {
                    System.out.println("잘못 선택하셨습니다. 다시 선택해주세요");
                ...
                for (int i = 0; i < 5; i++) {
                    if (burgers[i].burgerNum[i] >= 1) {
                        for (int k = 0; k < burgers[i].burgerNum[i]; k++) {
                            ...
                            burger_price[i] = burger_order[i].burgerPrice(i);
                            total_burger_price += burger_price[i];
                        }
                    }
                }
                // frozencustard
                for (int j = 0; j < 6; j++) {
                    if (frozen_custards[j].frozencustardNum[j] >= 1) {
                        for (int i = 0; i < frozen_custards[j].frozencustardNum[j]; i++) {
                            ...
                            frozen_custard_price[j] = frozen_custard_order[j].frozencustardPrice(j);
                            total_frozen_custard_price += frozen_custard_price[j];
                            }
                        }
                    }
                // drink
                for (int x = 0; x < 6; x++) {
                    if (drinks[x].drinkNum[x] >= 1) {
                        for (int i = 0; i < drinks[x].drinkNum[x]; i++) {
                            ...
                            drink_price[x] = drink_order[x].drinkPrice(x);
                            total_drink_price += drink_price[x];
                        }
                    }
                }
                // beer
                for (int y = 0; y < 2; y++) {
                    if (beers[y].beerNum[y] >= 1) {
                        for (int i = 0; i < beers[y].beerNum[y]; i++) {
                            ...
                            beer_price[y] = beer_order[y].beerPrice(y);
                            total_beer_price += beer_price[y];
                        }
                    }
                }
